...
import { APIError, ServiceResponse,Admin } from '../models'
import * as nodeUtil from 'util'

const TAG = 'authentication'

export function isAdminAuthenticated(req: any, res: Response, next: NextFunction): void {
  if (AUTHENTICATION.enabled) {
    ...
    try {
      ...
      req.Admin = decode.isAdmin;
      
      logger.debug('LOGGED IN USER:' + nodeUtil.inspect(req.admin))
      next()
    } catch (error) {
