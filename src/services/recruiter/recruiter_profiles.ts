...
export async function deleteRecruiterProfile(userID) {
  ...
  try {
    const deleteProfile =
      await RecruiterProfileDetailsData.deleteRecruiter(userID);
    if (deleteProfile) {
...
export async function uploadCompanyLogoFile(
  ...
  try {
    ...
    log.debug(
      ` ${TAG}.uploadCompanyLogoFile 's3 response:'` + nodeUtil.inspect(data)
    );
    log.debug(
      ` ${TAG}.uploadCompanyLogoFile 'fileS3 URL: ' ` +
        getFileUrl(data.savedFileKey, AWS_S3.BUCKET_NAME)
    );
    const fileDetails = {
    ...
    const fileSavedResp =
      await RecruiterProfileDetailsData.saveFile(fileDetails);
    ...
    serviceResponse.addServerError(
      "Failed to upload file due to technical difficulties"
    );
  }
  return serviceResponse;
}
...
export async function getRecruiterList(userID) {
  ...
  try {
    const existedProfile =
      await RecruiterProfileDetailsData.getRecruiterList(userID);
    if (existedProfile) {
      const data = {
        existedProfile,
      };
...
export async function updateCompanylogo(file: any, userID) {
  ...
  try {
    ...
    log.debug(
      ` ${TAG}.updateCompanylogo 's3 response:'` + nodeUtil.inspect(data)
    );
    log.debug(
      ` ${TAG}.updateCompanylogo 'fileS3 URL: ' ` +
        getFileUrl(data.savedFileKey, AWS_S3.BUCKET_NAME)
    );
    const fileDetails = {
    ...
    const updateImage = await RecruiterProfileDetailsData.updateCompanylogo(
      fileDetails,
      userID
    );
...
export async function uploadVideoFile(file: any): Promise<IServiceResponse> {
  ...
  try {
    ...
    const data = await saveFile(file, fileDirectory, AWS_S3.BUCKET_NAME);
    log.debug(
      ` ${TAG}.uploadVideoFile 's3 response:'` + nodeUtil.inspect(data)
    );
    log.debug(
      ` ${TAG}.uploadVideoFile 'fileS3 URL: ' ` +
        getFileUrl(data.savedFileKey, AWS_S3.BUCKET_NAME)
    );
    const fileDetails = {
      fileName: data?.savedFileName,
    ...
    const fileSavedResp =
      await RecruiterProfileDetailsData.uploadVideoFile(fileDetails);
    log.debug(
      ` ${TAG}.uploadCompanyInfoFile 'fileSavedResp response:'` +
        nodeUtil.inspect(fileSavedResp)
    );
